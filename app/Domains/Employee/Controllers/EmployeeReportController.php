...
class EmployeeReportController extends BaseApiController
{
    ...
    {
        $this->authorizeAbility($request, 'employee.reports.view');

        return $this->paginated(
            Employee::with(['department', 'position', 'branch'])
                ->orderBy('last_name')
                ->paginate(
                    min(
    ...
    public function transactions(Request $request, string $type)
    {
        $this->authorizeAbility($request, 'employee.reports.view');

        abort_unless(
            in_array($type, ['advance', 'deduction'], true),
            404
        );

        $rows = EmployeeTransaction::with('employee')
            ->where('type', $type)
            ->where('status', 'posted')
        ...
        return $this->paginated(
            $rows,
            EmployeeTransactionResource::class,
            ucfirst($type) . ' report retrieved successfully.'
        );
    }

    public function salaryHistory(Request $request, Employee $employee)
    {
        $this->authorizeAbility($request, 'employee.reports.view');

        return $this->success(
            $employee->salarySetups()
